...
| P-14 | Audit Delta Formation | required matrix + existing corpus | evidence delta/audit findings | Audit | 1, 2, 3 |
...
| Audit | Установить дельту и риски существующего корпуса | фактический корпус, ПД/РД, contract/customer rules, НТД | P-09, P-10, P-12, P-14, P-16 | создание operational work execution | финализирован audit delta с evidence и scope coverage | 1, 2, 3 по scope |
...
| `Audit` | fixed audit scope; actual corpus; applicable ПД/РД, contract, customer regulations and НТД; corpus inventory | K-01…K-09 + P-09/P-10/P-12/P-14/P-16 | pinned manifest с common rules и approved Audit completeness/delta/collision/trace rules | audit report; evidence delta; PD/RD/volume/payment findings; coverage and limitation statement | unavailable artifacts, sampling boundary, disputed authenticity/applicability, unresolved conflict; no claim outside scope | independent authorized auditor/reviewers and finalizer; source owners cannot self-resolve required SoD decisions | `AT-PE-43` плюс общие suites | audit scope/inventory зафиксированы; каждая finding/delta traced; coverage/limitations explicit; blockers resolved or terminally reported as blocking outcome |
...
Основные команды: `EstablishAuditScope`, `CompareRequiredAndActualEvidence`,
`ConfirmAuditFinding`, `FormAuditDelta`, `FinalizeAuditReport`. Ключевые
события: `AuditScopeEstablished`, `EvidenceGapDetected`,
`AuditFindingConfirmed`, `AuditDeltaFinalized`. Uncertainty сохраняет
границу выборки, недоступный источник и непроверенную применимость; audit
report не объявляет полноту вне установленного scope.
...
| AT-PE-43 | Audit E2E required-vs-actual corpus | inventory/scope/coverage явны; каждая delta/finding имеет evidence/rule trace; вывод вне scope запрещён |
