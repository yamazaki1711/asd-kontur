...
| G-05 Platform Knowledge Foundation | G-04 | Source/Evidence Ledger, official registry, NTD editions/units, canon, rules, FTS/vector/typed graph, Gateway, Promotion Gate | Knowledge/rule authorities; RuleVersion only qualified human | `IN_PROGRESS — local qualification PASS; canonical PostgreSQL 18 CI pending` |
...
`G-05 Platform Knowledge Foundation` выполняется в отдельной authorized ветке;
последний acceptance evidence — canonical PostgreSQL 18 + pgvector CI. До его
прохождения G‑05 не объявляется PASS и G‑06 не начинается.
