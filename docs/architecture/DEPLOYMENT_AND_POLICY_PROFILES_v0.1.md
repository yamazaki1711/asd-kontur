...
| `G02-E-008` | pre-rebaseline dependency evidence; current `pyproject.toml`, `uv.lock` | 2026-08-22…23 | Current G-04 project pins Python `>=3.12,<3.13`, PostgreSQL/Contract validation and test stack; pre-rebaseline renderer packages no longer belong to active baseline | Qualified DOCX/XLSX/PDF/DXF renderer stack; current dependencies его намеренно не содержат |
