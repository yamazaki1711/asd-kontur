# ASD-KONTUR documentation index

Start here:

1. [Product Goal and Capability Map](product/PRODUCT_GOAL_AND_CAPABILITY_MAP_v1.md)
2. [Four-mode Functional Model](product/FOUR_MODE_FUNCTIONAL_MODEL_v1.md)
3. [Architecture Blueprint](architecture/ARCHITECTURE_BLUEPRINT_v1.md)
4. [Implementation Plan](architecture/IMPLEMENTATION_PLAN_v1.md)
5. [Trial Readiness](product/TRIAL_READINESS_SPECIFICATION_v1.md)

Architecture detail:

- [Technical Architecture](architecture/TECHNICAL_ARCHITECTURE_v1.md)
- [Product Interaction](architecture/PRODUCT_INTERACTION_ARCHITECTURE_v1.md)
- [Industrial Intake](architecture/INDUSTRIAL_DOCUMENT_INTAKE_ARCHITECTURE_v1.md)
- [Project Understanding](architecture/PROJECT_UNDERSTANDING_ARCHITECTURE_v1.md)
- [Drawing/CAD boundary](architecture/DRAWING_CAD_INTELLIGENCE_BOUNDARY_v1.md)
- [Deployment and Operations](architecture/DEPLOYMENT_AND_OPERATIONS_ARCHITECTURE_v1.md)
- [Legacy Component Decision Matrix](architecture/LEGACY_COMPONENT_DECISION_MATRIX_v1.md)
- [ADR-0013](architecture/decisions/0013-platform-kernel-to-product-application.md)
- [ADR-0014](architecture/decisions/0014-product-application-technology-baseline.md)

Machine truth:

- [Contract Pack v2.0](../contracts/v2.0/README.md)
- [Product Application Spine Contract Pack v2.1](../contracts/v2.1/README.md)
- [Memory Integrity Contract Pack v2.2](../contracts/v2.2/README.md)
- [Industrial Document Understanding Contract Pack v2.3](../contracts/v2.3/README.md)
- [FIELD-ANDROID-CLIENT-01 technical debt](implementation/FIELD_ANDROID_CLIENT_01.md)
- [INDUSTRIAL-DOCUMENT-UNDERSTANDING-01 implementation record](implementation/INDUSTRIAL_DOCUMENT_UNDERSTANDING_01.md)
- [Product Goal Rebaseline Decision](product/PRODUCT_GOAL_REBASELINE_DECISION_v1.json)
- [Product Current State Decision v1.1](product/PRODUCT_CURRENT_STATE_DECISION_v1.1.json)
- [Product Current State Decision v1.2](product/PRODUCT_CURRENT_STATE_DECISION_v1.2.json)
- [Implementation record](implementation/PRODUCT_GOAL_REBASELINE_v1.md)
- [Product Application Spine implementation](implementation/PRODUCT_APPLICATION_SPINE_v0.1.md)
- [Memory Integrity Fix implementation](implementation/MEMORY_INTEGRITY_FIX_01.md)
- [Memory Integrity exact defect manifest](verification/MEMORY_INTEGRITY_DEFECT_MANIFEST_01.json)
- [Product Application Spine local runtime](operations/PRODUCT_APPLICATION_SPINE_LOCAL_RUNTIME_v0.1.md)

Earlier documents and reports are retained as historical evidence. A newer
version or superseding notice controls current readiness semantics.
