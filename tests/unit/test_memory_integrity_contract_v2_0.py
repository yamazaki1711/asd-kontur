...
def test_memory_integrity_contract_is_additive_and_fixtures_validate(
    ...
    validator = Draft202012Validator(
        schemas["urn:asd-kontur:contracts:v2.0:schema:memory-integrity"],
...
def test_readiness_and_empty_rule_registry_semantics_are_explicit(repository_root: Path) -> None:
    fixture = _load(
        repository_root / "contracts/v2.0/fixtures/valid/memory-integrity-counters.json"
    )
    ...
    readiness = load_module_readiness_manifest(
        repository_root / "contracts/v2.0/fixtures/valid/module-readiness-manifest.json"
    )
    rule_registry = next(
