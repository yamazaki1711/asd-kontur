...
def test_g07_contract_extension_is_immutable_valid_and_offline(repository_root: Path) -> None:
    root = repository_root / "contracts" / "v1.1"
    registry_document = json.loads((root / "registry.json").read_text(encoding="utf-8"))
    assert registry_document["registry_version"] == "1.1.0"
    ...
    references = tuple(_refs(schema))
    assert all(
        reference.startswith("#") or reference.startswith("urn:asd-kontur:contracts:")
