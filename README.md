...
> **Current product direction (superseding, 2026-09-25):** ASD-KONTUR is an
> applied construction-engineering system. Its primary result is an understood
> project, practical engineering/commercial conclusions, and useful documents.
> Processing, traceability, provenance, authorization, and readiness controls
> support that result; they are not the product or its primary user workflow.
...
**АСД-КОНТУР** — объектно-независимый прикладной программный комплекс для
понимания и инженерного анализа строительного проекта, выявления ошибок,
пропусков, противоречий и рисков, а также подготовки профессиональных
результатов и документов по объектам капитального строительства (**ОКС**).
...
Основной продуктовый конвейер:

`проектные документы → состав проекта → сооружения и участки → работы →`
`объёмы и материалы → сравнения и требования → проблемы и риски →`
`действия и профессиональные документы`.

Workspace, источники, версии, задания обработки, история решений, экспорт и
архив обеспечивают надёжность этого конвейера, но не заменяют его результат.

Четыре обязательных режима используют одно доменное ядро:

1. `Tender` — понять проект и коммерческий состав, сопоставить ПД/РД,
   спецификации, ВОР, смету и договор, выявить расхождения, риски и вопросы;
2. `Support` — определить и подготовить требуемую ИД по выполняемым работам,
   показать готовые и недостающие документы и исходные данные;
3. `Audit` — установить, что должно быть в существующем комплекте ИД, что
   фактически есть, что отсутствует, противоречит или требует исправления;
4. `Restoration` — безопасно восстановить возможную часть исторической ИД и
   точно назвать факты, которые ещё должен предоставить человек.
...
- Основная единица прогресса — решённая профессиональная задача пользователя,
  а не обработанный документ, кандидат, receipt, тест или статус готовности.
- Основной интерфейс строится от проекта: объект → площадка/сооружение →
  конструкция/котлован → работа → объём/материал → требование → проблема/риск
  → действие. Состояние обработки и диагностика находятся во вторичном
  техническом разделе.
- Capability становится содержательно полезной только тогда, когда пользователь
  получает её профессиональный результат через приложение на репрезентативных
  проектных данных. Схема БД, API, тест или успешно завершённая модельная задача
  сами по себе этого не доказывают.
- Ссылка на документ, лист или фрагмент помогает проверить инженерный вывод,
  но не является самим выводом.
