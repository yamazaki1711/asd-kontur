"""Recover dependency-terminal jobs from accepted successor lineages.

Revision ID: 0051_successor_recovery
Revises: 0050_project_structure_reconciliation_job
"""
...
revision = "0051_successor_recovery"
down_revision = "0050_project_structure_reconciliation_job"
branch_labels = None
depends_on = None


def upgrade() -> None:
    """Avoid scanning every historical blocked job when a worker is idle.

    A replacement is recoverable only when an equivalent successor has already
    succeeded. Start from that bounded set of accepted successors, walk its
    same-input causation lineage, then find terminal dependents. The old query
    expanded a recursive successor search for every historical blocked job
    before applying its ``LIMIT 16``; a large immutable history therefore
    prevented an otherwise idle worker from claiming runnable work.
    """
    op.execute(
        """
          ...
          FOR candidate IN
            WITH RECURSIVE successful_lineage AS MATERIALIZED (
              SELECT successor.organization_id, successor.workspace_id,
                     ...
                     successor.input_digest, successor.completed_at,
                     1 AS depth
              ...
              SELECT lineage.organization_id, lineage.workspace_id,
                     ...
                     predecessor.causation_id AS predecessor_job_id,
                     lineage.job_kind, lineage.subject_document_id,
                     lineage.input_digest, lineage.completed_at,
                     lineage.depth+1
            ...
            replacement AS MATERIALIZED (
              SELECT DISTINCT ON (organization_id, workspace_id, predecessor_job_id)
                     organization_id, workspace_id, predecessor_job_id,
                     replacement_job_id
...
def downgrade() -> None:
    ...
    source = Path(__file__).with_name("0045_bounded_dependency_recovery.py")
